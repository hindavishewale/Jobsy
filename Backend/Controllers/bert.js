...
// Similarity calibration settings
const SIMILARITY_CONFIG = {
  minExpected: 0.3,    // Minimum expected similarity for good matches
  maxExpected: 0.8,    // Maximum expected similarity
  boostFactor: 1.5,    // Factor to boost scores
  minThreshold: 0.1    // Minimum threshold to display
};
...
async function getBERTEmbedding(text, retryWithFallback = true) {
  try {
    ...
    // Fallback to the original model if primary fails
...
function cosineSimilarity(vecA, vecB) {
  ...
  // Handle different dimensionalities - ensure both are flat arrays
...
// Enhanced similarity calibration function
function calibrateSimilarityScore(rawSimilarity, modelType) {
  ...
  // Different calibration based on model type
  if (modelType === MODEL_CONFIG.primary) {
    // Fine-tuned model might produce lower absolute values but better relative rankings
    calibrated = Math.pow(rawSimilarity, 0.7); // Adjust the curve
...
// Create more focused and relevant text representations
...
function calculateAdditionalMatchFactors(candidate, internship) {
  ...
  // Skills overlap bonus (simple version)
  ...
  return Math.min(additionalScore, 0.5); // Cap additional score at 50%
}
...
async function matchInternships(candidateEmail) {
  try {
    ...
    for (let internship of filteredInternships) {
      ...
      try {
        // Use the same model for internship embedding that worked for candidate
        ...
        // Combine BERT similarity with additional factors
